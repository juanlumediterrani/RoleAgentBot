...
def build_canvas_role_treasure_hunter_detail(
    ...
    if detail_name in {"ring"}:
        ...
        status_label = general_messages.get("status", "Status:")
        active_text = general_messages.get("active", "✅ Active")
        inactive_text = general_messages.get("inactive", "❌ Inactive")
    ...
    if detail_name in {"ring_admin"}:
        ...
        treasure_hunter_messages = personality_descriptions.get("role_descriptions", {}).get("treasure_hunter", {})
        general_messages = personality_descriptions.get("general", {})
        ring_descriptions = treasure_hunter_messages.get("ring", {})

        ring_state = _get_canvas_ring_state(guild)

        admin_status = general_messages.get("status", "Status:")
        active_text = general_messages.get("active", "Active")
        inactive_text = general_messages.get("inactive", "Inactive")
        base_freq_label = general_messages.get("base_frequency", "Base Frequency:")
        current_freq_label = general_messages.get("current", "Current Frequency:")
        freq_format = general_messages.get("frequency_format", "Every {hours}h")
        hot_potato_format = ring_descriptions.get("hot_potato", "🔥 **Load Transfer:** Iteration {iteration} (Latency reduced {multiplier}x)")
        description = ring_descriptions.get("description", "Track and identify the target. Point to other users to help the algorithm resolve the anomaly.")
...
class TreasureHunterRingActionModal(discord.ui.Modal, title="Treasure Hunter Ring Action"):
    def __init__(self, action_name: str, author_id: int, guild, admin_visible: bool, view=None):
        ...
        super().__init__(title=titles.get(action_name, "Treasure Hunter Ring Action"))
        self.action_name = action_name
        self.guild = guild
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await handle_canvas_treasure_hunter_ring_modal_submit(
            interaction,
            self.action_name,
...
async def handle_canvas_treasure_hunter_ring_modal_submit(interaction: discord.Interaction, action_name: str, raw_value: str, guild, author_id: int, admin_visible: bool, view=None) -> None:
    """Handle ring modal submissions for treasure hunter."""
    import asyncio
    ...
    if action_name == "ring_accuse":
        try:
            from roles.treasure_hunter.subroles.ring.ring_discord import _get_ring_state, _save_ring_state, _record_accusation
            ...
            treasure_hunter_role = PERSONALITY.get("roles", {}).get("treasure_hunter", {})
            ring_subrole = treasure_hunter_role.get("subroles", {}).get("ring", {})
    ...
    if action_name in {"ring_on", "ring_off"}:
        ...
        # Save to server_config as subrole of treasure_hunter
        try:
            from .server_config import set_role_config_value
            set_role_config_value(server_id, "treasure_hunter", "config.subroles.ring.enabled", enabled)
            ok = True
        except Exception as e:
        ...
        if ok:
            ...
            # Refresh the view with navigation buttons, redirect to ring_admin after activation
            current_detail = "ring_admin" if enabled else "ring"
            content = _build_canvas_role_detail_view(
                "treasure_hunter",
                view.admin_visible,
                current_detail,
            ...
            role_embed = _build_canvas_role_embed("treasure_hunter", content, view.admin_visible, current_detail, None, None, server_id=server_id)
    ...
    if action_name == "ring_frequency":
        ...
        # Update frequency in server_config as subrole of treasure_hunter
        try:
            from .server_config import set_role_config_value
            set_role_config_value(server_id, "treasure_hunter", "config.subroles.ring.config.frequency_hours", hours)
            set_role_config_value(server_id, "treasure_hunter", "config.subroles.ring.config.base_frequency_hours", hours)
            set_role_config_value(server_id, "treasure_hunter", "config.subroles.ring.config.current_frequency_hours", hours)
            set_role_config_value(server_id, "treasure_hunter", "config.subroles.ring.config.frequency_iteration", 0)
            ok = True
        except Exception as e:
